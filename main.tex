...
\begin{itemize}
    \item Recall from our class that a Complete Partial Order (CPO) on a set $S$ is a partial order with a least element, s.t. for each increasing sequence $\{x_n\}$ there exists a least upper bound: $\bigsqcup_{n=0}^\infty x_n\in S$. 
    \item For a Hilbert space $\H$, the \emph{L\"{o}wner order} $\sqsubseteq$ is defined on the set of linear operators on $\H$ (which by definition contains $\dhnall$) by $A\sqsubseteq B\iff B-A\succeq 0$ (i.e., $B-A$ is a positive semidefinite operator). 
...
\end{definition}
\bigbreak
\textbf{Notation:} Let $\mathcal{P(H)}$ denote the set of all predicates in $\mathcal{H}$.
}
...
p(i) $\{wp\mathcal{(E)}(M)\} \mathcal{E} \{M\}$ \bigbreak
...
where on the right hand side we consider $\sem{S}:\dhall\to \dhall$ as a quantum operation. 
\end{enumerate}
}
...
Also cf.: $$\vDash_{tot} \{Q\}S\{P\}\implies \forall\rho.\tr(Q\rho)\leq \tr(P\sem{S}(\rho))$$
}
...
And likewise, the comparison $$\vDash_{par} \{Q\}S\{P\}\implies \forall\rho.\tr(Q\rho)\leq \tr(P\sem{S}(\rho))+\tr(\rho)-\tr(\sem{S}(\rho))$$ evinces the tightness of this liberal precondition.
}
...
\frame[t]{
\frametitle{Proof Systems for Partial Correctness Is Sound and Complete}
%We verify that 

By induction on program structures (as well as number of iterations in the case of the \textbf{while}-loop) with application of trace properties, one may find:
...
\end{theorem}
 %by induction on structure of the program $S$. It suffices to show that the three axioms are valid in the sense of partial correctness, and the inference rules preserve partial correctness.
...
\frame[t]{
\frametitle{Proof Systems for Total Correctness Is  Sound and Complete}
