...
\begin{itemize}
    \item The \textbf{trace} of a matrix $A$, denoted $tr(A)$ is the sum of the elements in the main diagonal (top left to bottom right)
    \bigbreak
...
\begin{lemma}
For any observables (and therefore any quantum predicates) $M$ and $N$:\bigbreak
$M\sqsubseteq N \iff tr(M\rho) \leq tr(N\rho)$ for all $\rho$\bigbreak
...
if $tr(M\rho) \leq tr(N\rho)$ for all $\rho$.
\end{definition}
...
\begin{itemize}
    \item $tr(M\rho) = 1, tr(N\rho)$ = 1 -- maybe valid
    \item $tr(M\rho) = 0, tr(N\rho)$ = 0 -- maybe valid
    \item $tr(M\rho) = 0.001, tr(N\rho)$ = 0.01 -- maybe valid
    \item $tr(M\rho) = 0.98, tr(N\rho)$ = 0.97 -- invalid!
